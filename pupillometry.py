...
class Pupillometry:
    ...
    def __init__(self):
        sns.set_style('ticks')
        self.blur_filters = ['Média', 'Mediana',
                             'Gaussiano', 'Equalização de histograma']
        self.high_filters = ['Prewitt vertical',
                             'Sobel vertical', 'Scharr vertical']
        self.morph_operators = [
    ...
    def get_summary_config(self):
        return pd.DataFrame(
            columns=['Valor'],
            index=[self.config_names[i] for i in list(self.config.keys())],
    ...
    def show_image_and_summary_table(self, image, cmap='gray'):
        ...
        return pd.DataFrame(
            columns=['Valor'],
            index=['Dimensão (linhas, colunas)', 'Valor mínimo de intensidade',
    ...
    def get_blur_filtered_images(self, selem, sigma, image):
        ...
        for type_of_filter in self.blur_filters:
            images_filtered[type_of_filter] = self.transform_image(
                image, selem, sigma, type_of_filter)
        return images_filtered

    def get_images_morphologically_transformed(self, selem, image):
        images_morph = dict()
        for type_of_operation in self.morph_operators:
            images_morph[type_of_operation] = self.transform_image(
                image, selem, None, type_of_operation)
        return images_morph
    ...
    def get_high_filtered_images(self, selem, image):
        ...
        for type_of_filter in self.high_filters:
            images_filtered[type_of_filter] = self.transform_image(
                image, selem, None, type_of_filter)
        return images_filtered
    ...
    def handle_flashes(self, L, k, image):
        artifacts = []
        lim_min, lim_row_max, lim_col_max, = L + \
            1, image.shape[0]-L-1, image.shape[1]-L-1
        # Para cada pixel (x, y)
        for r in range(lim_min, lim_row_max):
            for c in range(lim_min, lim_col_max):
                ...
                change_pixel = (v0 > vr*k) and (v0 > vl *
                                                k) and (vu > vr*k) and (vd > vr*k)
                if change_pixel:
    ...
    def pupil_segmentation_hough(self, image):
        ...
        _, cols, rows, rads = hough_circle_peaks(
    ...
    def draw_circle_perimeter(self, image, region):
        r_coords_pupil, c_coords_pupil = circle_perimeter(
            region[0], region[1], region[2], shape=image.shape)
    ...
    def iris_segmentation_horizontal_gradient(self, image, offset_col_low, offset_col_high):
        ...
        col_gradient = np.gradient(
            image[self.region_pupil[0], col_pos:(col_pos + offset_col_high)])
    ...
    def get_df_results_for_one_image(self):
        return pd.DataFrame(
            columns=['Valor'],
            index=['Centroide (linha, coluna)',
                   'Raio da Pupila (pixels)', 'Raio da Íris (pixels)'],
