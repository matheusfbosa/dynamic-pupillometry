...
def main():
    ...
    st.sidebar.info(
        'Sistema de detecção automática de pupila e íris em imagens de pupilometria dinâmica.')
    ...
    if mode == 'Completo':
        ...
        if st.sidebar.checkbox('Pré-Processamento'):
            if pup.image_processing is not None:
                ...
                st.warning(
                    'Execute a etapa de carregamento de imagem antes de proceder.')
        ...
        if st.sidebar.checkbox('Segmentação da pupila'):
            if pup.image_pre_processed is not None:
                ...
                st.warning(
                    'Execute a etapa de pré-processamento antes de proceder.')
        ...
        if st.sidebar.checkbox('Segmentação da íris'):
            if pup.region_pupil is not None:
                ...
                st.warning(
                    'Execute a etapa de segmentação da pupila antes de proceder.')
        ...
        if st.sidebar.checkbox('Resultados'):
            if pup.region_iris is not None:
                ...
                st.warning(
                    'Execute a etapa de segmentação da íris antes de proceder.')
...
def load_image():
    ...
    option_image = st.radio(
        'Selecione:', ('Realizar upload de imagens', 'Utilizar imagem de exemplo'))

    if option_image == 'Realizar upload de imagens':
        multiple_files_uploader()
        if len(pup.images_to_process) > 0:
            hashfile = st.selectbox(
                'Selecione uma das imagens carregadas:', tuple(pup.images_to_process.keys()))
            pup.image_processing = pup.images_to_process.get(hashfile)
        else:
            st.warning(
                'Selecione uma das imagens carregadas antes de proceder.')
        ...
        if pup.image_processing is not None:
            if st.checkbox('Mostrar imagem'):
                df = pup.show_image_and_summary_table(
                    image=pup.get_image_as_ubyte(next(iter(pup.images_to_process.values()))))
                st.pyplot()
                st.table(df)
        ...
        if st.checkbox('Mostrar lista de imagens', True):
            for index, hashfile in enumerate(pup.images_to_process.keys()):
                ...
                if st.checkbox('Mostrar imagem ' + str(index+1)):
                    df = pup.show_image_and_summary_table(
                        image=pup.images_to_process.get(hashfile))
                    st.pyplot()
                    st.table(df)
...
def run_all_steps():
    ...
    if len(pup.artifacts_found) > 0:
        st.success(
            f'Reflexos encontrados e tratados em: {str(pup.artifacts_found).strip("[]")}.')
    ...
    if pup.config['seg_pup_method'] == 'Binarização global':
        # Histograma
        _, bins = pup.get_histogram(
            image=image_pre_processed_ubyte, n_bins=pup.config['seg_pup_bin_nbins'])
    ...
    pup.region_iris = __iris_segmentation_horizontal_gradient(
        image=pup.image_filtered_high.copy())
    pup.bbox_iris = pup.get_bounding_box(region=pup.region_iris)
    st.success('Busca encerrada.')

    st.text(
        f'Tempo de processamento do imagem: {round(time.time() - start_time, 3)} segundos')
...
def pre_processing():
    ...
    if st.checkbox('Filtragem de suavização'):
        if pup.image_processing is not None:
            st.info(
                'Um filtro de suavização é utilizado com o objetivo de remover ruído e detalhes finos da imagem.')
            ...
            if pup.images_to_plot_blur is not None:
                ...
                pup.show_all_images(pup.images_to_plot_blur.values(
                ), nrows=2, ncols=2, titles=pup.blur_filters)
                st.pyplot()
                pup.config['pre_proc_filter_type'] = st.selectbox(
                    'Selecione o filtro com melhor resultado:', pup.blur_filters)
                pup.image_filtered_blur = pup.images_to_plot_blur.get(
                    pup.config['pre_proc_filter_type'])
            else:
    ...
    if st.checkbox('Tratamento de reflexos'):
        if pup.image_filtered_blur is not None:
            st.info('''A determinação do raio da pupila pode ser prejudicada devido a presença de reflexos causados por LEDs infravermelhos utilizados para sensibilização pupilar.
                    ...
                    ''')
            st.image(image='./resources/flash_parameters.png',
                     caption='Parâmetros L e k.', width=200)
            ...
            if len(pup.artifacts_found) > 0:
                ...
                st.success(
                    'Nenhum reflexo foi encontrado a partir dos parâmetros.')
...
def __handle_flashes(L, k, selem, image):
    ...
    image_pre_processed = pup.transform_image(
        image_handle_flashes, selem=selem, sigma=None, type_of_function='Mediana')
...
def pupil_segmentation():
    ...
    if st.checkbox('Mostrar imagem com pupila segmentada'):
        if pup.region_pupil is not None:
            ...
            pup.image_pupil = pup.draw_circle_perimeter(
                image=pup.image_pre_processed.copy(), region=pup.region_pupil)
            pup.show_image(pup.image_pupil)
            st.pyplot()

            st.text(
                f'Bounding box (min_row, min_col, max_row, max_col): {pup.bbox_pupil}')
            st.success(
                f'Pupila encontrada em ({pup.region_pupil[0]}, {pup.region_pupil[1]}) com raio de {pup.region_pupil[2]} pixels.')
...
def pupil_segment_global_binarization():
    ...
    pup.config['seg_pup_bin_nbins'] = st.slider(
        'Número de intervalos do histograma:', min_value=1, max_value=100, value=15)

    image_pre_processed_ubyte = pup.get_image_as_ubyte(pup.image_pre_processed)
    hist, bins = pup.get_histogram(
        image=image_pre_processed_ubyte, n_bins=pup.config['seg_pup_bin_nbins'])
    thresh = int(round(bins[1]))
    ...
    pup.show_image_and_histogram(image=image_pre_processed_ubyte,
                                 image_bin=image_bin, n_bins=pup.config['seg_pup_bin_nbins'])
    st.pyplot()
    ...
    if pup.images_to_plot_morph is not None:
        st.markdown('Resultado:')
        pup.show_all_images(pup.images_to_plot_morph.values(),
                            titles=pup.morph_operators, nrows=2, ncols=2)
        st.pyplot()

        pup.config['seg_pup_morph_type'] = st.selectbox(
            'Selecione a operação com melhor resultado:', pup.morph_operators)
        image_morph = pup.images_to_plot_morph.get(
            pup.config['seg_pup_morph_type'])

        image_labels, n_labels = pup.labelizer_image(image=image_morph)

        st.text(f'Número de regiões encontradas: {n_labels}')
        if st.checkbox('Mostrar imagem com rótulos'):
            image_labels_colored = pup.label_to_rgb(
                image_labels, image=pup.image_pre_processed)
...
def iris_segmentation():
    ...
    if st.button('Aplicar filtragem de realce'):
        pup.images_to_plot_high = __images_to_plot_high_filtering(
            selem, image=pup.image_pre_processed.copy())

    if pup.images_to_plot_high is not None:
        st.markdown('Resultado:')
        pup.show_all_images(pup.images_to_plot_high.values(),
                            nrows=1, ncols=3, titles=pup.high_filters)
        st.pyplot()

        pup.config['seg_iris_filter_type'] = st.selectbox(
            'Selecione o filtro com melhor resultado:', pup.high_filters)
        pup.image_filtered_high = pup.images_to_plot_high.get(
            pup.config['seg_iris_filter_type'])
    ...
    if pup.image_filtered_high is not None:
        ...
        if st.checkbox('Mostrar imagem com íris segmentada'):
            if pup.region_iris is not None:
                ...
                pup.image_iris = pup.draw_circle_perimeter(
                    image=pup.image_pre_processed.copy(), region=pup.region_iris)
                pup.show_image(pup.image_iris)
                st.pyplot()

                st.text(
                    'Bounding box (min_row, min_col, max_row, max_col): ' + str(pup.bbox_iris))
                st.success(
                    f'Íris encontrada em ({pup.region_iris[0]}, {pup.region_iris[1]}) com raio de {pup.region_iris[2]} pixels.')
...
def results_for_one_image():
    ...
    pup.image_processed = pup.draw_circle_perimeter(
        image=pup.image_processing.copy(), region=pup.region_pupil)
    pup.image_processed = pup.draw_circle_perimeter(
        image=pup.image_processed, region=pup.region_iris)
